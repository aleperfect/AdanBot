let handler = async (m, { conn, command, text, usedPrefix }) => {
  ...
  if (command == 'peruano') {
  conn.reply(m.chat, `
  _*${text.toUpperCase()}* *ES* *${(500).getRandom()}%* *${command.replace('how', '').toUpperCase()} Guardemos las palomas que el peruano las ara extinguir 😧*_
  `.trim(), m, m.mentionedJid ? {
  mentions: m.mentionedJid
  } : {})}
  if (command == 'peruana') {
  conn.reply(m.chat, `
  _*${text.toUpperCase()}* *ES* *${(500).getRandom()}%* *${command.replace('how', '').toUpperCase()} Guardemos las palomas que esta peruana las ara extinguir 😧*_
  `.trim(), m, m.mentionedJid ? {
  mentions: m.mentionedJid
  } : {})}
  if (command == 'chilena') {
  conn.reply(m.chat, `
  _*${text.toUpperCase()}* *ES* *${(500).getRandom()}%* *${command.replace('how', '').toUpperCase()} 🇨🇱🗣️: kusdufsudfuajfjsdfjsfjdjsdfjsjfsjdf    🌎: Habla despacio ctmre no te entendemos ni mrda*_
  `.trim(), m, m.mentionedJid ? {
  mentions: m.mentionedJid
  } : {})}
  if (command == 'chileno') {
